...
const text = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (cmd === 'welcome') {
    if (!m.isGroup) return m.reply("*📛 MR DASA THIS COMMAND CAN ONLY BE USED IN GROUPS*");
    ...
    if (!botAdmin) return m.reply("*📛 MR DASA BOT MUST BE AN ADMIN TO USE THIS COMMAND*");
    if (!senderAdmin) return m.reply("*📛 MR DASA BOT YOU MUST BE AN ADMIN TO USE THIS COMMAND*");
    let responseMessage;
